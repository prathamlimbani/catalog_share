-- =============================================================================
-- CatalogShare — the three migrations that were never applied to production.
--
-- WHY SYNC IS BROKEN
--
-- Verified against the live database on 21 Aug 2026. All three of these come
-- back 42703 "column does not exist":
--     invoices.advance_payment
--     invoices.updated_at
--     invoices.deleted_at
-- and both RPCs are absent (PGRST202):
--     next_invoice_number()
--     start_estimate_trial()
--
-- Every push sends advance_payment, so PostgREST rejects it with 42703. The
-- push path only recovers from 23505 (duplicate invoice number) -- any other
-- error is treated as "retry", so the entry burns its 8 attempts and parks.
-- The result is that NO estimate has ever reached the server, silently.
--
-- HOW TO APPLY
...
-- ###########################################################################
-- ## 20260730000000_add_advance_payment.sql
-- ###########################################################################
...
-- ###########################################################################
-- ## 20260819000000_mobile_app_sync_and_plans.sql
-- ###########################################################################
...
-- ###########################################################################
-- ## 20260819000002_invoice_soft_delete.sql
-- ###########################################################################
...
-- ###########################################################################
-- ## Realtime for the companies row
-- ##
-- ## Lets a client learn about its own plan changing the moment it happens,
-- ## instead of waiting for a cache to expire. This is what makes a payment
-- ## captured AFTER verification (the 202 "authorized, not captured" path) drop
-- ## the paywall on its own, and what makes an admin-granted plan appear on a
-- ## merchant's open screen.
-- ##
-- ## The client degrades gracefully without this -- it falls back to refreshing
-- ## on focus, resume and a five-minute interval -- so this is an improvement,
-- ## not a requirement.
-- ###########################################################################
...
-- Realtime sends the OLD row on UPDATE only with REPLICA IDENTITY FULL; without
-- it the payload's `old` is just the primary key. The client only reads `new`,
-- so this is left at the default -- noted here so nobody adds it by reflex.


-- ###########################################################################
-- ## Verification -- run this after the migration and check every row says OK
-- ###########################################################################

SELECT 'invoices.advance_payment'      AS thing,
       CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns
                          WHERE table_name='invoices' AND column_name='advance_payment')
            THEN 'OK' ELSE 'MISSING' END AS status
UNION ALL SELECT 'invoices.updated_at',
       CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns
                          WHERE table_name='invoices' AND column_name='updated_at')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL SELECT 'invoices.deleted_at',
       CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns
                          WHERE table_name='invoices' AND column_name='deleted_at')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL SELECT 'companies.trial_started_at',
       CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns
                          WHERE table_name='companies' AND column_name='trial_started_at')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL SELECT 'next_invoice_number()',
       CASE WHEN EXISTS (SELECT 1 FROM pg_proc WHERE proname='next_invoice_number')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL SELECT 'start_estimate_trial()',
       CASE WHEN EXISTS (SELECT 1 FROM pg_proc WHERE proname='start_estimate_trial')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL SELECT 'realtime on companies',
