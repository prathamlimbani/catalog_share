 ...
 * =====================
 * Ported from supabase/functions/create-razorpay-order/index.ts. The logic, the
 * order of the checks, the status codes and the error strings are unchanged;
 * only the Deno-isms moved: `serve()` became a default-exported handler,
 * `Deno.env.get` became `env()`, the esm.sh import became the npm package, and
 * the local corsHeaders/json copies now come from ./runtime.mjs (that shared
 * corsHeaders also sends Access-Control-Allow-Methods, which the local copy did
 * not). The caller-scoped client is built by callerClient(authHeader), which is
 * the same createClient call the Deno file made by hand.
 *
...
import { env, corsHeaders, json, callerClient } from "./runtime.mjs";
 ...
 */
async function planPriceRupees(client, planId) {
 ...
 */
async function resolveCoupon(client, planId, priceRupees, couponCode, _companyId) {
