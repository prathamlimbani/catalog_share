 ...
 *   - pdf-lib was a top-level `https://esm.sh/pdf-lib@1.17.1` import. It is now
 *     the npm package, imported dynamically inside generateInvoicePdf: this
 *     module shares a process with payment verification (see index.mjs), and a
 *     static import of a package that is not installed would stop the whole
 *     host from booting rather than failing the one branch that needs a PDF.
 *     pdf-lib is not in package.json today, so the admin_plan_change branch
 *     returns its usual 500 until it is installed; every other type is fine.
 *
 * Everything else - the templates, the wording, the order of checks, the status
 * codes - is carried across unchanged.
 */
...
function welcomeEmailHtml(companyName) {
  return `
<!DOCTYPE html>
<html>
<head>
...
function invoiceEmailHtml(payload) {
...
function expiryReminderEmailHtml(payload) {
  const expiryDate = payload.expiresAt
    ? new Date(payload.expiresAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })
    : "soon";
...
function adminNewCompanyHtml(companyName, companyEmail) {
...
function adminNewSubscriptionHtml(companyName, companyEmail, planName, amount, expiresAt) {
...
function planChangeEmailHtml(payload) {
...
async function generateInvoicePdf(payload) {
  // Was a top-level `import { PDFDocument, rgb, StandardFonts } from
  // "https://esm.sh/pdf-lib@1.17.1"`. Deferred to here so a missing pdf-lib
  // fails this one request instead of the whole function host.
  // NOTE: pdf-lib is NOT currently installed - this branch (admin_plan_change)
  // will 500 until `npm i pdf-lib` is run on the host. esm.sh served it as ESM;
  // the npm package is CJS, so take `.default` (Node sets it to module.exports)
  // and fall back to the namespace, rather than relying on named-export detection.
  const pdfLib = await import("pdf-lib");
  const { PDFDocument, rgb, StandardFonts } = pdfLib.default ?? pdfLib;
...
function planStatusEmailHtml(companyName, currentPlan, expiresAt) {
...
function planDowngradedEmailHtml(companyName, previousPlan) {
  return `
<!DOCTYPE html>
<html>
<head>
...
function adminPlanDowngradedHtml(companyName, companyEmail, previousPlan) {
  const now = new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata", dateStyle: "medium", timeStyle: "short" });
  return `
<!DOCTYPE html>
<html>
...
export default async function handler(req) {
  ...
  try {
    ...
    switch (type) {
      // ADDED BEYOND THE PORT (deliberately, not drift): the admin console has a
      // "send test email" button, and there was no type that could prove the
      // credentials work without also sending a real customer a real welcome
      // or invoice email. Plain text, no template, no side effects.
      case "test":
        subject = "CatalogShare test email";
        html = [
          ...
          'This was sent from the CatalogShare admin console to confirm the SMTP ',
          'credentials are correct. If you received it, password resets and ',
          'transactional email will work.',
          '</p>',
          '<p style="margin:0;color:#888;font-size:13px">Sent ' + new Date().toISOString() + '</p>',
          '</div>',
        ].join("");
        break;
      case "welcome":
